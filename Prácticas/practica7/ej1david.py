...
img1 = ski.io.imread("Prácticas/practica7/images/monedas1.png")
img2 = ski.io.imread("Prácticas/practica7/images/monedas2.png")
img3 = ski.io.imread("Prácticas/practica7/images/monedas3.png")
imagenes = (img1, img2, img3)
resultados = []
for img in imagenes:
    #filtered_image = ski.filters.rank.mean(ski.util.img_as_ubyte(img), footprint=ski.morphology.footprint_rectangle((8,8)))
    ...
    #img_umbralizada = ski.morphology.binary_erosion(img_umbralizada, footprint=st_elem)

    ##### Tema 9 #####
    st_elem = ski.morphology.disk(2)
    #img_umbralizada = ski.filters.rank.median(ski.util.img_as_ubyte(img_umbralizada), footprint=ski.morphology.footprint_rectangle((6,6)))
    img_cierre = ski.morphology.binary_closing(img_umbralizada, footprint=st_elem)
    ##################
    st_elem = ski.morphology.disk(11)
    st_elem2 = ski.morphology.disk(7)
    img_cierre = ski.morphology.binary_erosion(img_cierre, footprint=st_elem)
    img_cierre = ski.morphology.binary_dilation(img_cierre, footprint=st_elem2)
    ...
    contador_monedas = 0
    for p in props:
        if p.eccentricity < 0.6:
            if p.area < 750: # 10 centimos
                img_monedas[img_etiquetada == p.label] = 1
                print("moneda")
            else:
                img_monedas[img_etiquetada == p.label] = 2
            contador_monedas += 1
    print(f"Detectadas {contador_monedas} monedas")
    resultados.append(img_monedas)
